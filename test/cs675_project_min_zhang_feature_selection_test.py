...
separate_index=random.sample(range(len(train_data)),4000)
...
selection_value=feature_selection(train_data_1)

#feature_index=find_max(selection_value,num_feaure)

#####save separation index and index on file######
path=label_path = file_path+'/separate_index_test'
f = open(path, "w")
for item in separate_index:
  f.write("%s\n" % int(item))
f.close()

path=label_path = file_path+'/selection_value_test'
f = open(path, "w")
for item in selection_value:
  f.write("%s\n" % item)
f.close()

print("separate_index and feature_index saved on file")
